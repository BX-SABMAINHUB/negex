...
export const getTemplates = async (category?: string) => {
  ...
  if (category) q = query(q, where('category', '==', category)) as any;
  const snap = await getDocs(q);
...
export const createProject = async (userId: string, data: any) => {
  const ref = await addDoc(collection(db, 'projects'), { ...data, userId, createdAt: Timestamp.now(), updatedAt: Timestamp.now() });
  return ref.id;
};
